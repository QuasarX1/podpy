...
for key in data:
    for ion in data[key]:
        data[key][ion]["obs"]["centre"][0] = np.mean(np.array([data[key][ion]["obs"]["centre"][0], data[key][ion]["obs"]["upper_error"][0], data[key][ion]["obs"]["lower_error"][0]]).T, axis = 1)
        data[key][ion]["obs"]["upper_error"] = data[key][ion]["obs"]["upper_error"][1] - data[key][ion]["obs"]["centre"][1]
        data[key][ion]["obs"]["lower_error"] = data[key][ion]["obs"]["centre"][1] - data[key][ion]["obs"]["lower_error"][1]
...
def plot_obs(dataset: str, ion: str, colour = "red", label = None, axis = None):
    ...
    plotting_function(data[dataset][ion]["obs"]["centre"][0],
                      ...
                      yerr = np.array([data[dataset][ion]["obs"]["lower_error"], data[dataset][ion]["obs"]["upper_error"]]),
                      fmt = "o",
                      linestyle = "",
                      capsize = 2,
                      color = colour,
...
def plot_synthetic_combined(ion: str, colour = None, label = "Turner+16 Combined Model", axis = None):
    kwargs = {}
    kwargs["dataset"] = "combined"
    kwargs["ion"] = ion
...
def plot_obs_combined(ion: str, colour = None, label = "Turner+16 Combined Obs.", axis = None):
    kwargs = {}
    kwargs["dataset"] = "combined"
    kwargs["ion"] = ion
    if colour is not None:
...
def plot_synthetic_Q1317_0507(ion: str, colour = None, label = "Turner+16 Q1317-0507 Model", axis = None):
    kwargs = {}
    kwargs["dataset"] = "Q1317-0507"
    kwargs["ion"] = ion
    if colour is not None:
        kwargs["colour"] = colour
    kwargs["label"] = label
